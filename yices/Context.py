...
class Context(object):
    ...
    def __init__(self, config=None):
        ...
        self.context = yices.yices_new_context(cfg)


    # option is a string
    def enable_option(self, option):
        assert self.context is not None
        errcode = yices.yices_context_enable_option(self.context, option)
        return True if errcode == 0 else False

    # option is a string
    def disable_option(self, option):
        assert self.context is not None
        errcode = yices.yices_context_disable_option(self.context, option)
        return True if errcode == 0 else False


    def status(self):
        assert self.context is not None
        return yices.yices_context_status(self.context)


    def assert_formula(self, term):
        assert self.context is not None
        # FIXME: this idiom is wrong because Sam's stuff throws exceptions.
        errcode = yices.yices_assert_formula(self.context, term)
        if errcode == 0:
            return True
    ...
    def assert_formulas(self, python_array_or_tuple):
        ...
        a = yices.make_term_array(python_array_or_tuple)
        errcode = yices.yices_assert_formulas(self.context, alen, a)
        if errcode == 0:
            return True
    ...
    def check_context(self, params=None):
        assert self.context is not None
        return  yices.yices_check_context(self.context, params)
    ...
    def assert_blocking_clause(self):
        assert self.context is not None
        errcode = yices.yices_assert_blocking_clause(self.context)
        if errcode == 0:
            return True
    ...
    def push(self):
        assert self.context is not None
        errcode = yices.yices_push(self.context)
        if errcode == 0:
            return True
    ...
    def pop(self):
        assert self.context is not None
        errcode = yices.yices_pop(self.context)
        if errcode == 0:
            return True
    ...
    def check_context_with_assumptions(self, params, python_array_or_tuple):
        alen = len(python_array_or_tuple)
        a = yices.make_term_array(python_array_or_tuple)
        return yices.yices_check_context_with_assumptions(self.context, params, alen, a)

    def get_unsat_core(self):
        retval = []
        unsat_core = yices.term_vector_t()
        yices.yices_init_term_vector(unsat_core)
        errcode = yices.yices_get_unsat_core(self.context, unsat_core)
        if errcode == 0:
    ...
    def dispose(self):
        yices.yices_free_context(self.context)
        self.context = None
