...
class Model(object):
    ...
    GEN_DEFAULT    = yices.YICES_GEN_DEFAULT
    GEN_BY_SUBST   = yices.YICES_GEN_BY_SUBST
    GEN_BY_PROJ    = yices.YICES_GEN_BY_PROJ
    ...
    def from_context(context, keep_subst):
        return Model(yices.yices_get_model(context.context, keep_subst))
    ...
    def from_map(mapping):
        ...
        return Model(yices.yices_model_from_map(len(dom), yices.make_term_array(dom), yices.make_term_array(rng)))


    def collect_defined_terms(self):
        defined_terms = yices.term_vector_t()
        yices.yices_init_term_vector(defined_terms)
        yices.yices_model_collect_defined_terms(self.model, defined_terms)
        retval = []
        for i in range(0, defined_terms.size):
            retval.append(defined_terms.data[i])
        yices.yices_delete_term_vector(defined_terms)
        return retval

    def dispose(self):
        assert self.model is not None
        yices.yices_free_model(self.model)
        self.model = None


    def get_bool_value(self, term):
        ytval = ctypes.c_int32()
        errcode = yices.yices_get_bool_value(self.model, term, ytval)
        if errcode == -1:
    ...
    def get_integer_value(self, term):
        ...
        errcode = yices.yices_get_int64_value(self.model, term, ytval)
        if errcode == -1:
    ...
    def get_fraction_value(self, term):
        ...
        errcode = yices.yices_get_rational64_value(self.model, term, ytnum, ytden)
        if errcode == -1:
    ...
    def get_float_value(self, term):
        ...
        errcode = yices.yices_get_double_value(self.model, term, ytval)
        if errcode == -1:
    ...
    def get_scalar_value(self, term):
        """ Returns the index of the value. This is the low level version, and does not use yices.yices_constant. """
        ytval = ctypes.c_int32()
        errcode = yices.yices_get_scalar_value(self.model, term, ytval)
        if errcode == -1:
    ...
    def formula_true_in_model(self, term):
        return True if yices.yices_formula_true_in_model(self.model, term) == 1 else False

    def formulas_true_in_model(self, term_array):
        tarray = yices.make_term_array(term_array)
        return True if yices.yices_formulas_true_in_model(self.model, len(term_array), tarray) == 1 else False


    def get_value_from_rational_yval(self, yval):
        if yices.yices_val_is_int64(self.model, yval):
            val = ctypes.c_int64()
            errcode = yices.yices_val_get_int64(self.model,  yval, val)
            if errcode != 0:
                sys.stderr.write('Model.get_value: yices_val_get_int64 failed {0}\n', yices.yices_error_string())
                return None
            return val.value
        elif yices.yices_val_is_rational64(self.model, yval):
            ytnum = ctypes.c_int64()
            ytden = ctypes.c_int64()
            errcode = yices.yices_val_get_rational64(self.model,  yval, ytnum, ytden)
            if errcode != 0:
                sys.stderr.write('Model.get_value: yices_val_get_int64 failed {0}\n', yices.yices_error_string())
                return None
            return Fraction(ytnum.value, ytden.value)
            ...
            val = ctypes.c_double()
            errcode = yices.yices_val_get_double(self.model,  yval, val)
            if errcode != 0:
                sys.stderr.write('Model.get_value: yices_val_get_int64 failed {0}\n', yices.yices_error_string())
                return None
            return val.value
    ...
    def get_value_from_scalar_yval(self, yval):
        ...
        errcode =  yices.yices_val_get_scalar(self.model, yval, value, typev)
        if errcode != 0:
            sys.stderr.write('Model.get_value: yices_val_get_scalar failed {0}\n', yices.yices_error_string())
            return None
        return yices.yices_constant(typev.value, value.value)

    def get_value_from_bv_yval(self, yval):
        bvsize = yices.yices_val_bitsize(self.model, yval)
        if bvsize <= 0:
            return None
        bvarray = yices.make_empty_int32_array(bvsize)
        errcode = yices.yices_val_get_bv(self.model, yval, bvarray)
        if errcode != 0:
    ...
    def get_value_from_tuple_yval(self, yval):
        tuple_size = yices.yices_val_tuple_arity(self.model, yval)
        if tuple_size <= 0:
            return None
        yval_array = yices.make_empty_yval_array(tuple_size)
        errcode = yices.yices_val_expand_tuple(self.model, yval, yval_array)
        if errcode == -1:
            return None
    ...
    def get_value_from_mapping_yval(self, yval):
        mapping_size = yices.yices_val_mapping_arity(self.model, yval)
        if mapping_size <= 0:
            return None
        ytgt = yices.yval_t()
        ysrc = yices.make_empty_yval_array(mapping_size)
        errcode = yices.yices_val_expand_mapping(self.model, yval, ysrc, ytgt)
        if errcode == -1:
            return None
    ...
    def get_value_from_function_yval(self, yval):
        function_size = yices.yices_val_function_arity(self.model, yval)
        if function_size <= 0:
            return None
        ydefault = yices.yval_t()
        ymapping = yices.yval_vector_t()
        yices.yices_init_yval_vector(ymapping)
        errcode = yices.yices_val_expand_function(self.model, yval, ydefault, ymapping)
        if errcode == -1:
            yices.yices_delete_yval_vector(ymapping)
            return None
    ...
    def get_value(self, term):
        ...
        yval = yices.yval_t()
        errcode = yices.yices_get_value(self.model, term, yval)
        if errcode == -1:
    ...
    def get_value_as_term(self, term):
        return yices.yices_get_value_as_term(self.model, term)

    def implicant_for_formula(self, term):
        retval = []
        termv = yices.term_vector_t()
        yices.yices_init_term_vector(termv)
        yices.yices_implicant_for_formula(self.model, term, termv)
        retval = []
        for i in range(0, termv.size):
            retval.append(termv.data[i])
        yices.yices_delete_term_vector(termv)
        return retval
    ...
    def implicant_for_formulas(self, term_array):
        ...
        termv = yices.term_vector_t()
        yices.yices_init_term_vector(termv)
        yices.yices_implicant_for_formulas(self.model, len(term_array), tarray, termv)
        retval = []
        for i in range(0, termv.size):
            retval.append(termv.data[i])
        yices.yices_delete_term_vector(termv)
    ...
    def generalize_model(self, term, elim_array, mode):
        var_array = yices.make_term_array(elim_array)
        termv = yices.term_vector_t()
        yices.yices_init_term_vector(termv)
        errcode = yices.yices_generalize_model(self.model, term, len(elim_array), var_array, mode, termv)
        if errcode == -1:
            yices.yices_delete_term_vector(termv)
            return None
        retval = []
    ...
    def generalize_model_array(self, term_array, elim_array, mode):
        ...
        termv = yices.term_vector_t()
        yices.yices_init_term_vector(termv)
        errcode = yices.yices_generalize_model_array(self.model, len(term_array), tarray, len(elim_array), var_array, mode, termv)
        if errcode == -1:
            yices.yices_delete_term_vector(termv)
            return None
        retval = []
